...
DeMixT.S1 <- function(inputdata, groupid, niter = 10, ninteg = 50, filter.option = 1, filter.criteria1 = c(0.5,0.5), filter.criteria2 = c(250,250), filter.criteria3 = 0.25, if.filter = FALSE){
...
#filter.criteria2 can be an integer or quantile
	...
	inputdatamat2 = inputdata
	   ...
	   res <- DeMixT.Kernel(inputdatamat2, groupid, nhavepi=0, givenpi=rep(0, 2*sum(groupid==3)), givenpiT=rep(0,sum(groupid==3)), niter=niter, ninteg=ninteg)	
	}else{
...
#filter.criteria2 can be an integer or quantile
        ...
        # Run this function
		res1 <- DeMixT.Kernel(inputdatamat2, cnvgroup, nhavepi=0, givenpi=rep(0, sum(groupid==3)), givenpiT=rep(0,sum(groupid==3)), niter=niter, ninteg=ninteg)
...
#filter.criteria2 can be an integer or quantile
        ...
        #step 2
		res <- DeMixT.Kernel(inputdatamat3, groupid, nhavepi = 2, givenpi=rep(0, 2*sum(groupid==3)), givenpiT = fixed.piT, niter=niter, ninteg=ninteg)
		print("Filtering stage 2 over")
	}

	  pi <- t(as.matrix(res$pi[1,])); row.names(pi)[1] = "pi1";
      
	  pi1 <- as.matrix(res$pi1); colnames(pi1) = as.character(1:niter); row.names(pi1) = sample.id
	  pi_iteration <- list(); pi_iteration[[1]] <- pi1
	  if(nCid == 2) {
          pi <- rbind(pi, res$pi[2,])
          row.names(pi)[2] = "pi2"
		  pi2 <- as.matrix(res$pi2);colnames(pi2) = as.character(1:niter); row.names(pi2) = sample.id
          pi_iteration[[2]] <- pi2
				}
